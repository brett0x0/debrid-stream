...
export class StreamHandler {
    ...
    cache;
    publicBaseUrl;
    constructor(orchestrator, rdClient, cache, publicBaseUrl) {
    ...
    async getStreams(type, id, config, encodedConfig, baseUrlOverride) {
        ...
        for (const h of hashes) {
            ...
            const isCached = await this.cache.get(availKey);
            if (isCached === true) {
                cachedHashes.add(h);
            }
        ...
        const streams = ranked.map((item) => {
            ...
            if (sizeStr)
                specs.push(`💾 ${sizeStr}`);
            if (p?.codec && p.codec !== 'unknown')
                specs.push(`⚙️ ${p.codec}`);
            if (p?.hdr?.dolbyVision)
                specs.push('✨ DV');
            else if (p?.hdr?.hdr)
                specs.push('✨ HDR');
            ...
            if (c.seeders > 0)
                specs.push(`👥 ${c.seeders}`);
            if (c.provider)
                specs.push(`🔗 ${c.provider.toUpperCase()}`);
            if (specs.length > 0) {
