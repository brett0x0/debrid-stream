...
export class RealDebridClient {
    ...
    timeoutMs;
    constructor(timeoutMs = env.RD_REQUEST_TIMEOUT_MS) {
    ...
    async getInstantAvailability(hashes, token) {
        ...
        for (const chunk of chunks) {
            ...
            try {
                const resp = await this.request(`/torrents/instantAvailability/${hashPath}`, token);
     ...
     */
    async addMagnet(magnet, token) {
        return this.request('/torrents/addMagnet', token, {
            method: 'POST',
            body: { magnet },
        });
    }
     ...
     */
    async selectFiles(torrentId, fileIds, token) {
        await this.request(`/torrents/selectFiles/${torrentId}`, token, {
            method: 'POST',
            body: { files: fileIds },
        });
    }
     ...
     */
    async getTorrentInfo(torrentId, token) {
        return this.request(`/torrents/info/${torrentId}`, token);
    }
    /**
     * Converts a hoster/torrent file link into an unrestricted streaming URL.
     */
    async unrestrictLink(link, token) {
        return this.request('/unrestrict/link', token, {
            method: 'POST',
            body: { link },
        });
    }
     ...
     */
    async resolvePlayableStream(magnetUri, token, fileIdx) {
        // 1. Add magnet
